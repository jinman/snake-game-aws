...
public class LoginActivity extends Activity implements Session.StatusCallback,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Initialize the Google Play API Client for Login
		...
		btnLogin = (ImageButton) findViewById(R.id.btnLWALogin);
		btnLogin.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				loginRequestType = LoginType.AWS;
				try {
					authManager = new AmazonAuthorizationManager(
							LoginActivity.this, Bundle.EMPTY);
					authManager.authorize(APP_SCOPES, Bundle.EMPTY,
							new AuthListener());
				} catch (Exception e) {
					e.printStackTrace();
		...
		if (session == null) {
			btnFBLogin.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					loginRequestType = LoginType.FACEBOOK;
		...
		// Adding Google Play Login Button Click event functionality
		...
		/*
		 * btnGoogleLogin = (ImageButton)findViewById(R.id.btnGoggleLogin);
		 * btnGoogleLogin.setOnClickListener(new OnClickListener() {
		 * 
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case GOOGLE:
			if (requestCode == RC_SIGN_IN) {
				// Following G+ documentation, but this seems pointless
...
//"oauth2:" + Scopes.PLUS_LOGIN
				...
				AWSClientManager.addLogins("accounts.google.com", accessToken);
				break;
			case FACEBOOK:
				Log.i(LOG_TAG, "facebook token: " + session.getAccessToken());
				AWSClientManager.addLogins("graph.facebook.com",
